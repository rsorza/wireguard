...
new_client_type () {
	echo "Select the client type:"
	echo "   1) Linux / generic WireGuard client"
	echo "   2) MikroTik RouterOS"
	read -p "Client type [1]: " client_type
	until [[ -z "$client_type" || "$client_type" =~ ^[1-2]$ ]]; do
		echo "$client_type: invalid selection."
		read -p "Client type [1]: " client_type
	done
	[[ -z "$client_type" ]] && client_type="1"
}
...
configure_ufw () {
	...
	if [[ -n "$default_iface" ]]; then
		ufw route allow in on wg0 out on "$default_iface"
	else
		ufw route allow in on wg0
	fi
	sed -i 's/^DEFAULT_FORWARD_POLICY=.*/DEFAULT_FORWARD_POLICY="ACCEPT"/' /etc/default/ufw
	if ! grep -q '^# BEGIN_WIREGUARD_NAT$' /etc/ufw/before.rules; then
		sed -i "1i# BEGIN_WIREGUARD_NAT\n*nat\n:POSTROUTING ACCEPT [0:0]\n-A POSTROUTING -s 10.7.0.0/24 ! -d 10.7.0.0/24 -j SNAT --to-source $ip\nCOMMIT\n# END_WIREGUARD_NAT\n" /etc/ufw/before.rules
	fi
	if [[ -n "$ip6" ]] && ! grep -q '^# BEGIN_WIREGUARD_NAT$' /etc/ufw/before6.rules; then
		sed -i "1i# BEGIN_WIREGUARD_NAT\n*nat\n:POSTROUTING ACCEPT [0:0]\n-A POSTROUTING -s fddd:2c4:2c4:2c4::/64 ! -d fddd:2c4:2c4:2c4::/64 -j SNAT --to-source $ip6\nCOMMIT\n# END_WIREGUARD_NAT\n" /etc/ufw/before6.rules
	fi
	ufw --force enable
	ufw reload
}

remove_ufw_wireguard_rules () {
	default_iface=$(ip -4 route list default | awk '{print $5; exit}')
	if [[ -e /etc/ufw/before.rules ]]; then
		sed -i '/^# BEGIN_WIREGUARD_NAT$/,/^# END_WIREGUARD_NAT$/d' /etc/ufw/before.rules
	fi
	if [[ -e /etc/ufw/before6.rules ]]; then
		sed -i '/^# BEGIN_WIREGUARD_NAT$/,/^# END_WIREGUARD_NAT$/d' /etc/ufw/before6.rules
	fi
	ufw --force delete allow "$port"/udp 2>/dev/null
	if [[ -n "$default_iface" ]]; then
		ufw route delete allow in on wg0 out on "$default_iface" 2>/dev/null
	else
		ufw route delete allow in on wg0 2>/dev/null
	fi
...
new_client_setup () {
	...
	if [[ "$client_type" == "2" ]]; then
		server_public_key=$(grep PrivateKey /etc/wireguard/wg0.conf | cut -d " " -f 3 | wg pubkey)
		endpoint=$(grep '^# ENDPOINT' /etc/wireguard/wg0.conf | cut -d " " -f 3)
		listen_port=$(grep ListenPort /etc/wireguard/wg0.conf | cut -d " " -f 3)
		echo
		echo "MikroTik setup"
		echo "1. Open MikroTik Terminal and create a WireGuard interface:"
		echo "   /interface wireguard add name=wg-$client disabled=no"
		echo "2. Show the generated public key:"
		echo "   /interface wireguard print detail where name=wg-$client"
		echo "3. Copy the public-key value from MikroTik and paste it below."
		echo
		read -p "MikroTik Public Key: " mikrotik_public_key
		until [[ -n "$mikrotik_public_key" ]]; do
			echo "Public key is required."
			read -p "MikroTik Public Key: " mikrotik_public_key
		done
		default_mikrotik_allowed_ips="10.7.0.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/128")"
		echo
		echo "Enter AllowedIPs for this MikroTik peer on the Linux server."
		echo "For site-to-site, add the MikroTik LAN here, for example: $default_mikrotik_allowed_ips, 192.168.88.0/24"
		read -p "AllowedIPs [$default_mikrotik_allowed_ips]: " mikrotik_allowed_ips
		[[ -z "$mikrotik_allowed_ips" ]] && mikrotik_allowed_ips="$default_mikrotik_allowed_ips"
		echo
...
PresharedKey = $psk
AllowedIPs = $mikrotik_allowed_ips
# END_PEER $client
EOF
		cat << EOF > "$script_dir"/"$client"-mikrotik.rsc
# MikroTik RouterOS commands for peer "$client"
# Run these commands on the MikroTik after creating the WireGuard interface.

/ip address add address=10.7.0.$octet/24 interface=wg-$client
/interface wireguard peers add interface=wg-$client public-key="$server_public_key" preshared-key="$psk" endpoint-address="$endpoint" endpoint-port=$listen_port allowed-address=$mikrotik_peer_allowed persistent-keepalive=25s
EOF
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	# Install a firewall if the selected firewall is not already available
	if [[ "$os" == "debian" || "$os" == "ubuntu" ]]; then
		echo
		echo "Select firewall backend:"
		echo "   1) UFW"
		echo "   2) iptables"
		read -p "Firewall [1]: " firewall_choice
		until [[ -z "$firewall_choice" || "$firewall_choice" =~ ^[1-2]$ ]]; do
			echo "$firewall_choice: invalid selection."
			read -p "Firewall [1]: " firewall_choice
		done
		[[ -z "$firewall_choice" ]] && firewall_choice="1"
	...
	# Install WireGuard using distro packages and the kernel implementation
	if [[ "$os" == "ubuntu" ]]; then
		# Ubuntu
		apt-get update
...
WantedBy=multi-user.target" >> /etc/systemd/system/wg-iptables.service
	...
	if [[ "$client_type" == "2" ]]; then
		echo "Finished!"
		echo
		echo "MikroTik RouterOS commands are available in:" "$script_dir"/"$client"-mikrotik.rsc
	else
	...
	case "$option" in
		1)
			...
			echo
			if [[ "$client_type" == "2" ]]; then
				echo "$client added. MikroTik RouterOS commands available in:" "$script_dir"/"$client"-mikrotik.rsc
			else
				qrencode -t ANSI256UTF8 < "$script_dir"/"$client.conf"
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				elif grep -qs '^# BEGIN_WIREGUARD_NAT$' /etc/ufw/before.rules /etc/ufw/before6.rules 2>/dev/null; then
					remove_ufw_wireguard_rules
				else
